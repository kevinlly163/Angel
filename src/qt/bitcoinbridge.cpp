...
bool BitcoinBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        switch(rcp.txnTypeInd)
        {
            ...
            case TXT_ANON_TO_BTC:
                formatted.append(tr("<b>%1</b> ANON, ring size %2 to HPA %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::SCR_AmountWithFeeExceedsAnonBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your anon Hpacoin balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
QVariantMap BitcoinBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Hpacoin prefix.");
            return result;
        }
